...
function setLoadingEnvironments(nextLoading) {
    ...
    loadEnvironmentsButton.disabled = running || loadingEnvironments;
    loadEnvironmentsButton.textContent = loadingEnvironments ? "Loading environments…" : "Load environments";
}
...
function formatSummary(payload) {
    ...
    if (Number.isInteger(summaryPayload.publishedCount)) {
        const action = summaryPayload.action === "unpublish" ? "Unpublished" : "Published";
...
function normalizeLogPayload(rawPayload) {
    ...
    return {
        message: typeof rawPayload.message === "string" ? rawPayload.message : JSON.stringify(rawPayload),
...
form.addEventListener("submit", async event => {
    ...
    if (rememberTokenInput.checked) {
        appendLog("Secure token storage is not enabled yet in this build. The token is only used in-memory for this run.", "info");
    }
