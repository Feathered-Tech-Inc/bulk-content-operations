...
import { type PublishAction, DEFAULT_BULK_ACTION_CONCURRENCY, MAX_BULK_ACTION_CONCURRENCY, getErrorMessage, parseAction, runPublishJob } from "./publish-core";
...
export function parseArgs(argv: string[]): CliOptions {
    ...
    if (!environmentId) {
        throw new Error("--environment is required (e.g. --environment your-environment-id)");
    }
...
export function resolveToken(cliToken?: string): string {
    ...
    if (!token) {
        throw new Error("token is required (use --token or set CONTENTFUL_CMA_TOKEN in .env/environment)");
    }
