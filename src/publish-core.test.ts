...
function createClientWithEntries(entries: EntryProps[], bulkStatus: unknown, onBulkAction?: (action: "publish" | "unpublish") => void): PlainClientAPI {
    return {
        entry: {
...
test("parseAction validates unknown actions", () => {
    ...
    assert.throws(() => parseAction("remove"), /Unknown --action value\. Valid actions: publish, unpublish/);
});
...
test("runPublishJob marks partial publish failures in summary", async () => {
    ...
    assert.equal(result.summary.results.find(item => item.entryId === "entry-2")?.status, "failed");
});
