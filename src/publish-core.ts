...
export async function loadAccessibleSpaces(params: {
  token: string;
}): Promise<PublishSpace[]> {
...
export async function loadSpaceEnvironments(params: {
  token: string;
  spaceId: string;
}): Promise<PublishEnvironment[]> {
  const token = params.token.trim();
...
export async function loadTagsForScope(params: {
  token: string;
  spaceId: string;
  environmentId: string;
}): Promise<PublishTag[]> {
...
export async function loadTargetTags(params: {
  token: string;
  target: PublishTargetId;
}): Promise<PublishTag[]> {
  throw new Error(
    `Legacy target aliases are deprecated and not supported for tag discovery (${params.target}). Use explicit spaceId and environmentId with loadTagsForScope instead.`,
  );
}

function emitLog(
  hooks: PublishJobHooks | undefined,
  level: PublishJobLog['level'],
  message: string,
  meta?: Record<string, unknown>,
): void {
...
async function collectTaggedEntries(
  ...
  emitLog(
    hooks,
    'error',
    `Fetching entries tagged ${tagId} to ${actionLabels.infinitive}...`,
  );
  ...
  while (true) {
    ...
    const query: {
      ...
      ? { limit: CURSOR_PAGE_SIZE, pageNext }
      : { 'metadata.tags.sys.id[in]': tagId, limit: CURSOR_PAGE_SIZE };
      ...
      if (needsAction(entry, action)) {
        ...
        if (publishableEntries.length >= maxPublishable) {
          emitLog(
            hooks,
            'error',
            `  page ${page}: fetched ${scannedCount} entries (stopped early after finding ${maxPublishable} ${actionLabels.noun})`,
          );
          emitProgress(hooks, {
            stage: 'collecting',
            page,
...
function buildBulkPublishPayload(
  entries: EntryProps[],
...
async function waitForBulkAction(
  client: PlainClientAPI,
  config: AppConfig,
  bulkActionId: string,
  pollSlot: number,
  ...
  throw new Error(
    `BulkAction ${bulkActionId} did not finish within ${(BULK_ACTION_MAX_POLLS * BULK_ACTION_POLL_MS) / 1000}s`,
  );
}

function parseBulkActionResults(
  entries: EntryProps[],
  bulkAction: BulkActionProps<BulkActionPublishPayload>,
  action: PublishAction,
): PublishResult[] {
...
async function publishBatch(
  ...
  if (batchCount > 1) {
    emitLog(
      hooks,
      'error',
      `Starting bulk ${actionLabels.infinitive} batch ${batchNumber}/${batchCount} (${entries.length} entries)...`,
    );
  } else {
    emitLog(
      hooks,
      'error',
      `Starting bulk ${actionLabels.infinitive} for ${entries.length} entries...`,
    );
  }
  ...
  const bulkAction = await waitForBulkAction(
    client,
    config,
    bulkActionInProgress.sys.id,
    (batchNumber - 1) % concurrency,
    hooks,
  );
    ...
    for (const result of results) {
      if (result.status !== 'failed') {
        emitLog(
          hooks,
          'info',
          `${actionLabels.past[0].toUpperCase()}${actionLabels.past.slice(1)} ${result.entryId} (${result.contentType})`,
        );
      } else {
        emitLog(
          hooks,
          'error',
          `Failed ${result.entryId} (${result.contentType}): ${result.error}`,
        );
      }
    }
  }
...
async function publishEntries(
  ...
  if (batches.length > 1) {
    emitLog(
      hooks,
      'error',
      `Splitting ${entries.length} entries into ${batches.length} bulk ${actionLabels.infinitive} actions (max ${BULK_ACTION_MAX_ITEMS} per action)...`,
    );
  }
  ...
  if (concurrency > 1 && batches.length > 1) {
    emitLog(
      hooks,
      'error',
      `Running up to ${concurrency} bulk actions in parallel (Contentful allows ${MAX_BULK_ACTION_CONCURRENCY} active per space)...`,
    );
  }

  const batchResults = await mapWithConcurrency(batches, concurrency, (batch, index) =>
    publishBatch(
      client,
      config,
      batch,
...
function logEntry(entry: EntryProps, verbose: boolean, hooks?: PublishJobHooks): void {
  ...
  emitLog(
    hooks,
    'info',
    `  ${id} (${contentType.sys.id}) version=${version} publishedVersion=${publishedVersion ?? 'none'}`,
  );
}
...
function normalizePublishJobParams(
  params: PublishJobParams,
): NormalizedPublishJobParams {
  const token = params.token.trim();
  ...
  if ((providedSpaceId && !providedEnvironmentId) || (!providedSpaceId && providedEnvironmentId)) {
    throw new Error(
      'spaceId and environmentId must be provided together',
    );
  }
...
export async function runPublishJob(
  ...
  if (fullyScanned) {
    ...
    emitLog(
      hooks,
      'info',
      `  ${publishableEntries.length} need ${actionLabels.gerund}`,
    );
    emitLog(
      hooks,
      'info',
      `  ${scannedCount - publishableEntries.length} already up to date (skipped)`,
    );
  } else {
    ...
    emitLog(
      hooks,
      'info',
      `  At least ${publishableEntries.length} need ${actionLabels.gerund} (scan stopped after reaching --limit)`,
    );
  }
  ...
  if (publishableEntries.length > 0) {
    emitLog(
      hooks,
      'info',
      `  Limit: ${normalizedParams.limit} (${normalizedParams.dryRun ? 'listing' : actionLabels.gerund} first ${limitedEntries.length} of ${publishableEntries.length})`,
    );
  }
  ...
  const results = await publishEntries(
    client,
    config,
    limitedEntries,
    config.action,
    normalizedParams.concurrency,
    normalizedParams.verbose,
    hooks,
  );
