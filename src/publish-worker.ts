...
export function parsePayload(raw: unknown): WorkerCommand {
  ...
  if (command !== 'run') {
    throw new Error(
      'command must be one of: run, load-spaces, load-environments, load-tags',
    );
  }
  ...
  const dryRun =
    payload.dryRun === undefined
      ? false
      : parseBoolean(payload.dryRun, 'dryRun');
  const verbose =
    payload.verbose === undefined
      ? false
      : parseBoolean(payload.verbose, 'verbose');
