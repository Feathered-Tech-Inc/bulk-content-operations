...
function getExpectedChecksum(shasumsText) {
  ...
  assert(
    checksumLine,
    `Could not find checksum for ${ARCHIVE_NAME} in ${SHASUMS_PATH}. Verify NODE_VERSION and PLATFORM.`,
  );

  const [checksum] = checksumLine.split(/\s+/);
  assert(
    /^[a-f0-9]{64}$/.test(checksum),
    `Invalid checksum format for ${ARCHIVE_NAME} in ${SHASUMS_PATH}.`,
  );
...
async function verifyArchiveChecksum() {
  ...
  assert(
    ...
    `Checksum mismatch for ${ARCHIVE_PATH}. Expected ${expectedChecksum}, got ${actualChecksum}. Delete ${ARCHIVE_PATH} and rerun.`,
  );
...
async function extractAndInstallRuntime() {
  ...
  try {
    ...
    assert(
      await fileExists(extractedNodePath),
      `Unable to extract ${tarEntryPath} from ${ARCHIVE_PATH}.`,
    );
